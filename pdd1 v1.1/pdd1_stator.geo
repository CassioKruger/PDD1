...
Point(dP+2) = {(Cos(ang_aux)*rinterno_estator),(Sin(Pi-aux2)*rr1), 0, 1.0};			//dP = 8
Point(dP+3) = {(Cos(-ang_aux)*rinterno_estator),(Sin(Pi+aux2)*rr1), 0, 1.0};			//dP = 9
...
For rad In {ang_ini_deg*deg2rad:ang_fin_deg*deg2rad:step_deg*deg2rad}
	Point(dP)={(Cos(rad)*rexterno_estator),(Sin(rad)*rexterno_estator),0,1.0};
	dP ++;
EndFor

For dL In {dL:(ang_fin_deg/step_deg)+dL-1:1}
...
//loop para criar a borda interna do estator
For rad In {ang_ini_deg*deg2rad:ang_fin_deg*deg2rad:step_deg*deg2rad}
	Point(dP)={(Cos(rad)*rinterno_estator),(Sin(rad)*rinterno_estator),0,1.0};
	dP ++;
EndFor
